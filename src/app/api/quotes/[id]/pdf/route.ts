...
export async function GET(
  ...
  try {
    ...
    doc.text("COTIZACIÓN", headerRightX, yPos + 5, { align: "right" })
    
    doc.setFontSize(16)
    doc.setTextColor(...textColor)
    doc.setFont("helvetica", "bold")
    doc.text(quote.id, headerRightX, yPos + 12, { align: "right" })
    ...
    doc.setFontSize(14)
    doc.setFont("helvetica", "bold")
    doc.text(quote.Client.name, pageWidth - margin, clientY, { align: "right" })
    ...
    if (quote.dueDate) {
      ...
      doc.text("VÁLIDA HASTA:", margin, yPos)
